...
**L4 continuation — 14 September:** L4-T1 is implemented and locally verified: Pages now has a per-account/project first-run checklist tied to saved design, created pages, a successfully rendered preview and verified live publication. Dismissal persists; existing websites keep their normal Pages screen. All 659 unit/integration tests pass, full types report no errors/warnings, and all 12 affected browser journeys have passing final outcomes, including real publication, rollback and unpublishing. Desktop/phone screenshots and the correction history are in the task map. Continue with **L4-T2**, the six-template gallery. Implementation is 19/32 tasks, about 59% by equal count; T1 is not deployed and the live release remains L3 `8a9c94c`.
