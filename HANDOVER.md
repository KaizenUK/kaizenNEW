...
**Latest scope update:** Sean has waived Firefox and Safari acceptance. They are no longer blockers. The real hosted canvas/on-page editing check remains open; see the [scope update](docs/existing-site-visual-editing-plan.md#scope-update--12-september-2026). Earlier references to required Firefox/Safari checks below are historical.

The pause has ended at Sean's request. The existing-site canvas implementation is deployed as `9aca128` / `gh-34712289074-1` and is in this checkout: website routes open from Pages into the real built page, with direct text editing, link addresses, image replacement, section ordering, shared outline/undo history, draft recovery, review/apply and an explicit commit step. The old field editor and separate selector are no longer the primary journey. This addresses the **critical correction in code**. Actual hosted acceptance was attempted, but the Codex browser left the loopback frame blank; the original goal must not be declared complete yet. The real local fallback passed selected-text/canvas synchronization, undo/redo, reviewed apply and rebuild, followed by byte-for-byte restoration of the temporary About heading. Browser controls could inspect the frame but could not deliver direct clicks/keys into it, so that manual input check remains open.
...
The framed preview uses per-canvas nonce paths and origin checks without third-party cookies, keeps network/form calls disabled, and preserves the previous snapshot until the next is verified. Isolated HTTPS framing passed Chromium 153 and Firefox 155 on Linux. Chrome requires its Local Network Access permission as well as helper folder approval. The actual hosted frame/input check, hosted Firefox and Safari on Sean's Mac remain open; see [the task map's acceptance log](docs/existing-site-visual-editing-plan.md#browser-support--implementation-log-12-september-2026). A helper follow-up avoids moving unchanged sections, with a regression assertion preserving hydrated islands. A disposable copy of Kaizen's real About page also passed direct desktop/phone typing with 281 fields across 18 files. Local verification passed: Astro/TypeScript checks, 214 unit tests, 40 browser scenarios (one opt-in licensed-archive skip), and the complete site/Studio production build. The historical pause-state sections below describe 11 September, not current implementation or deployment status.
