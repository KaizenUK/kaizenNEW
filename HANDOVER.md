...
**Hosted helper operations — 13 September:** the permanent `kaizen-hosted-helper.service` is now enabled and serving the corrected T7 helper source from `/opt/kaizen-helper-releases/3688a08`. Its health and restart checks pass; Sean's modified About page, saved draft and source recovery were preserved byte-for-byte. The private pre-upgrade project backup is retained on the VPS. Runtime-link startup is covered by a new real subprocess test. Frontend/branch rollout and the final CI recovery-browser check follow in the task log; the website artifacts below remain the current deployed frontend until those finish.

**Latest launch deployment — 13 September:** `e747452` is live in production (`gh-34770924919-1`) and staging (`gh-34770525644-1`), with all 259 HTTPS origin checks passing for each. The four matching builder functions and hosted helper are deployed. Codex used the signed-in production builder to build Kaizen's real About page on the VPS and inspect its rendered desktop/phone previews, without a local helper or content changes. That rollout used `kaizen-helper-rollout-e747452.service` and `/opt/kaizen-helper-releases/e747452`; the permanent T7 service above now replaces it. The managed source checkout retains that staging revision and Sean's pending About-page edit. The replacement Sanity credential is installed and verified, with no further token action pending. Continue the uncommitted L1-T7 operations work in the launch worktree, then the ordered beta tasks. Sean's later provisional acceptance is recorded above; this rollout is not completion of the launch goal.
