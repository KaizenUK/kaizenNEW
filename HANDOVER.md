...
**L2 continuation — 13 September:** email invitations, the owner member directory, invited-user setup and **L2-T2 role-based view defaults** are implemented and locally verified in `kaizen-launch`. Editors use client view; owners can choose developer details in Settings, remembered per account and project in this browser. The latest full checkpoint passes 497 tests and type checking with no errors or warnings. Six relevant browser journeys pass, with the three new view journeys repeated after screenshot review corrected the switch layout. Desktop/phone screenshots are retained in the task map. The L2 migration/functions/frontend have not been deployed. Continue with **L2-T3**, the account page, including Sean's missing-name/email Save notice, then T4 sign-in states. File/branch/commit hiding remains the explicit L3 work.
