...
export async function openSiteProject(
  ...
  if (hosted) {
    ...
    await page.route("**/client/lib/supabase.ts*", (route) =>
      route.fulfill({
        contentType: "application/javascript",
        body: `let session=${JSON.stringify(session)};const listeners=new Set();const client={auth:{getSession:async()=>({data:{session}}),refreshSession:async()=>({data:{session}}),signOut:async()=>{session=null;for(const fn of listeners)fn('SIGNED_OUT',null);return {error:null}},onAuthStateChange:callback=>{listeners.add(callback);return {data:{subscription:{unsubscribe:()=>listeners.delete(callback)}}}}},functions:{invoke:async(name,options)=>{const response=await fetch('/__fixture-projects',{method:'POST',headers:{'Content-Type':'application/json',...options.headers},body:JSON.stringify(options.body)});return {data:await response.json(),error:response.ok?null:{message:'Fixture project request failed'}};}}};export const getSupabaseClient=()=>client;`,
      }),
    );
    if (hostedWorkspace)
