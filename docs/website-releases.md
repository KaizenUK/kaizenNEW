...
| `BUILDER_HOSTED_GITHUB_READ_TOKEN` | Optional server-only GitHub token with Actions read access to the configured private repository, used only to observe staging deployment runs. Never put it in client configuration or a build environment. |
| `ALLOWED_STUDIO_ORIGINS` | Explicit allowed editor origins, with the parser/defaults described above. |

The repository file has `version: 1` and a `projects` array of `{ projectId, repositoryUrl, branch, saveToWebsite? }`. It supports explicit `git@hostname:owner/repository.git` SSH addresses and ordinary branch names. Local/file/HTTP URLs, URL credentials, executable protocol extensions and Git-option inputs are refused. No repository URL, branch or credential path comes from an editor's repository request. Project settings and public-key provisioning in the interface remain L1-T6; these are operator settings for now.
