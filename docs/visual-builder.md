...
The launch branch uses the hosted helper by default for a signed-in project. It connects to the website folder configured for that project; a saved folder preference from this computer is ignored. **Export & handoff → Hosted helper** shows the connection and offers **Refresh connection**. The client transport is implemented in L1-T1; the real hosted service, build queue, protected preview delivery and Kaizen setup follow in L1-T2–T6. Do not deploy this default until those services are ready. The current production acceptance above still describes the paired local helper.
