...
## Helper-window stability follow-up — 12 September 2026

- The next real hosted build exposed an additional M1-T4 failure: Astro reloaded the helper approval page during the website build. Its existing `pagehide` handler correctly revoked the capability, so the hosted editor lost its connection. The source file was unchanged and the draft remained recoverable.
- The local helper now serves the existing consent UI as an in-memory standalone JS/CSS bundle on the same route. It contains no Vite live-reload client. It keeps the existing folder approval, message checks, session expiry and disconnect logic; builds and source reloads no longer replace that browser document. Normal website development pages retain live reload. The bundle reads no environment files and writes no build output.
- `companion-reload.spec.ts` broadcasts an actual Vite full reload: the control page reloads, the approved helper window stays at the same document and handles another request, and explicit disconnect revokes the fixture session. The existing real-folder companion test also passes with this UI. Both focused browser tests passed. The final uninterrupted browser suite passed **42 scenarios**, with one optional licensed-archive skip (6.6 minutes); all **214 unit tests** passed. The complete Astro/TypeScript check reported 321 files, zero errors and zero warnings (172 existing hints).
- The next actual-checkout run preserved the window but exposed a separate server restart. The log reported “Configuration file removed” when `build:site` deleted `.astro/settings.json`; Astro's preference watcher restarts its server for that removal. The build now clears `dist` and lets Astro sync its own generated files, preserving the active server's preferences. Development watching also excludes the two retained recovery directories, which had produced a flood of HTML reload events.
- **Corrected real build:** the hosted About editor completed `pnpm run build`, including the site and Studio, while the standalone helper window remained connected. The helper log contained no build-induced server restart. The fresh framed route returned HTTP 200 with the original heading, the deployed image/Undo bridge, and `connect-src 'none'; form-action 'none'; frame-ancestors 'self' https://kaizenweb.co.uk`. The in-app browser still left this fresh, reachable frame blank; that remaining browser-access failure is independent of an expired snapshot. Studio's generated output was also excluded from development watching after this check, matching the recovery-output exclusions.

- **Release:** the stable approval window and build-preferences fix were deployed as `9aca128010aadd03aa9f45a4a88c56b74a87ea36`, release `gh-34712289074-1`. [Deployment 34712289074](https://github.com/KaizenUK/kaizenNEW/actions/runs/34712289074) succeeded and the public release marker matched. The local fallback subsequently built and rendered the actual About page again. Browser automation found its visible heading but failed both coordinate and frame-locator clicks with “Click target is no longer available”; direct manual input remains unverified, with the original source hash unchanged.
- **Server-stop detection:** the standalone window can outlive the server process, so its existing window heartbeat was insufficient. It now checks the existing local companion endpoint every ten seconds, with a five-second timeout, and disconnects the parent when the server or its exact session is unavailable. The check does not renew expiry or read folder contents; the token stays in the helper window. Unit assertions cover unknown, wrong-project, revoked and expired sessions. The lifecycle browser test now closes its actual isolated Vite server while leaving the approved popup open, and verifies that the parent receives the disconnect. That test and the real-folder edit/reconnect test both passed (2 tests, 36.7 seconds); TypeScript passed. The subsequent unit run passed 213 tests, with the opt-in Nginx test skipped because its binary variable was omitted in that invocation; that unchanged test passed in the immediately preceding 214-test run. This final check changes the local helper only and is saved without another public deployment.

## Final code verification and documentation audit — 12 September 2026

- At `d7d9359`, the complete Astro/TypeScript check passed (321 files, zero errors/warnings, 172 existing hints), all 214 unit tests passed with the Nginx binary configured, and the uninterrupted full browser suite passed 42 scenarios with one optional licensed-archive skip (6.9 minutes). Desktop/phone canvas screenshots were regenerated and visually checked. This closes the final-code verification gap after the server-stop change.
- M4-T4 documentation was reconciled throughout both guides: obsolete source-selector instructions and statements that existing pages cannot open in the canvas were replaced with Pages → Build → on-page edit → Review → Apply → optional Commit. Current helper labels, server-stop behaviour, exact Git limits and the remaining browser acceptance are explicit. Historical local-only/undeployed claims were removed from the current verification section.
- M0-T2 and the full real-site manual journey remain open. The signed-in hosted tab was rechecked with a live approved helper: the build had completed, but the frame remained `about:blank`. No new browser permission or manual Firefox/Safari result was received. The goal is not complete.
...
## Polish pass on the site editor — 12 September 2026

Claude Code reviewed the shipped editor against the Acme Dental fixture and applied a copy and layout pass. No behaviour, file boundary or Git rule changed; every existing site, source-editing, existing-pages and companion browser scenario still passes, as do the unit tests and the TypeScript check.

- **Plain names instead of tags.** `client/visual-builder/sourceLabels.ts` turns source labels into words people use: Heading, Text, Link text, Link address, Image, Image description, Button, Title. The outline, the Selected panel and the Sections tab all use it, as does the older source outline. Section groups read "Page sections", "Header items", "Navigation links" or "Section starting “…”". The technical location (tag, line, file) stays under "Where it comes from".
- **Builder-style panel tabs.** Outline/Assets/Sections and Selected/Page now use the shared `Segmented` control instead of bare buttons.
- **Canvas.** The preview frame fills the canvas height rather than stopping at 800px. Hover and selection outlines use the builder purple; the reorder control is a small "Move ↑ ↓" pill instead of two unstyled buttons on every selected element.
- **Pages.** "Pages from the website's code" moves above the "Add a page" banner as soon as a project has such pages (one slot, ordered with CSS so the card does not remount), gets a one-line description, and its button reads "Edit page". Its accessible name is unchanged.
- **Review dialog.** Centred and right-sized, close button top-right, first file open by default. Each file lists the actual edits as before/after text (and reordered groups); the whole file is available under "Show the whole file".
- **Notices and footer.** The applied/committed message is a styled notice with Dismiss, and it settles to "The preview shows them now" once the rebuild finishes. The commit form is a tidy row with a placeholder and a primary button; the status pill is wide enough for "Edits saved on this computer".
- **Copy.** "Only text, links and images can change here. The design stays as it is." replaces the registered-component sentence; the build card explains that the build runs on this computer and nothing goes live.
