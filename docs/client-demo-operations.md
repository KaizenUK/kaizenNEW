...
- `/etc/kaizen/client-destinations.json` contains the explicit binding. `/opt/kaizen-builder` is an operator-managed symlink to `/opt/kaizen-builder-releases/1179886`, containing commit `117988601806c4b403ad9398157d4c7680e68fe8` and its frozen dependencies (Vite 8.3.0). The earlier tooling directory is retained at `/opt/kaizen-builder-before-1179886`. Its release verifier retains the Cloudflare AJAX-header fix. The new compiler passed the standalone four-file/backup check as the service user on the VPS before activation; the timer was paused while switching an idle worker and restarted afterward. A scheduled run then completed with status 0. Production registry, credentials and private job/release stores remain external to these tooling releases.
