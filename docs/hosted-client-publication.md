...
Production installation (11 September 2026): `/opt/kaizen-builder`, system user `kaizen-builder`, private environment `/etc/kaizen/client-worker.env`, registry `/etc/kaizen/client-destinations.json`, and private work directory `/var/lib/kaizen-client-worker/jobs`. The systemd service has completed successfully and its timer is enabled. The registry now binds the explicitly approved `client-demo.kaizenweb.co.uk` production destination to its own release store. See [client demo operations](client-demo-operations.md) for live acceptance evidence and HTTPS renewal. The destination cannot overwrite Kaizen's CI release store.
...
This path has local PostgreSQL/worker protocol coverage, a standalone renderer-loader check and real process-crash recovery against isolated Nginx. The migrations and functions are now installed on the selected Supabase project, and real unrelated-account API/database/private-storage isolation checks pass. The provisioned client demo now passes supervised production publication, private-media download, failed-release restoration, rollback/unpublication and newer-draft retention; see its operations record. Recovery is an explicit operator action on the original host; the browser cannot reclaim worker ownership.
