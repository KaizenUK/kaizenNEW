import "../lib/ensureBrowserProcess";
...
const POST_STACK_ANCHOR_FIELDS = new Set(["field-body"]);
const MAIN_EDITOR_FIELDS = new Set(["field-title", "field-body"]);
...
export default function Studio() {
  ...
  // Presentation tool: make Enter key in the address bar navigate the preview
  useEffect(() => {
    function handleKeyDown(e) {
      ...
      // Sanity's Presentation address bar uses type="url".
      // Also catch text inputs inside any Presentation tool container.
      const isUrlInput = target.type === "url";
      const isInPresentation = Boolean(
        target.closest('[data-ui="PresentationTool"]') ||
        target.closest('[data-testid*="presentation"]') ||
        target.closest('[data-tool="presentation"]'),
      );
      const valueIsPath =
        target.value.trim().startsWith("/") ||
        target.value.trim().startsWith("http");

      if (!isUrlInput && !(isInPresentation && valueIsPath)) return;

      const url = target.value.trim();
      if (!url) return;

      // Find the Presentation preview iframe
      const iframe =
        document.querySelector('[data-testid="presentation-iframe"]') ||
        document.querySelector('iframe[src*="/preview"]') ||
        document.querySelector('iframe[src*="localhost"]') ||
        document.querySelector('iframe[src*="kaizen"]') ||
        document.querySelector('#sanity iframe');
      ...
      if (iframe instanceof HTMLIFrameElement && iframe.src) {
        ...
        try {
          ...
          if (url.startsWith("http")) {
            ...
            iframeUrl.pathname = url.startsWith("/") ? url : `/${url}`;
            iframeUrl.search = "";
